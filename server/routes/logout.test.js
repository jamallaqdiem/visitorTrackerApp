...
describe("POST /exit-visitor/:id", () => {
  test("should successfully log out a visitor and return 200", async () => {
    ...
    const visitorId = await new Promise((resolve, reject) => {
      mockDb.run(
        `INSERT INTO visitors (first_name, last_name) VALUES ('Jamal' , 'Laqdiem')`,
        function (err) {
          if (err) return reject(err);
          mockDb.run(
            `INSERT INTO visits (visitor_id, entry_time) VALUES (?, ?)`,
            [this.lastID, new Date().toISOString()],
            (err) => {
              if (err) return reject(err);
              resolve(this.lastID);
            }
    ...
    expect(response.body).toHaveProperty(
      "message", `Jamal Laqdiem has been successfully signed out.`
    );

    // Verify the exit time was set in the database
    const visit = await new Promise((resolve, reject) => {
      mockDb.get(
        `SELECT * FROM visits WHERE visitor_id = ? AND exit_time IS NOT NULL`,
        [visitorId],
        (err, row) => {
          if (err) return reject(err);
          resolve(row);
  ...
  test("should return 404 for a non-existent visitor ID", async () => {
    ...
    expect(response.body).toHaveProperty(
      "message",
      "Visitor not found or already signed out."
    );
  });
});
